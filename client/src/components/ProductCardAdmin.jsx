...
const ProductCardAdmin = ({data, fetchProductData }) => {
  ...
  return (
    <div className='w-36 p-4 bg-slate-100 rounded-md'>
        <div>
            <img
             src={data?.image[0]} alt={data?.name} 
        ...
        <div className='grid gap-3 grid-cols-2 py-2'>
          <button onClick={()=>setEditOpen(true)} className='border text-sm p-1 border-green-600 bg-green-100 text-green-800 hover:bg-green-300 rounded'>Edit</button>
          <button onClick={()=>setOpenDelete(true)} className='border text-sm p-1 border-red-600 bg-red-100 text-red-800 hover:bg-red-300 rounded'>Delete</button>
        </div>
        {
          editOpen && 
